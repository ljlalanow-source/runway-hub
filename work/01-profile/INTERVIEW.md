...
**사전 인터뷰 웹사이트에서 정해 온다.** 사용자가 [RUNWAY 사전 인터뷰](https://claude.ai/artifact/AH6NCWH1qJ1j7eym34KCFu)의 "결과 복사" 내용(`RUNWAY 사전 인터뷰 · 이니셜` + `profile: { name, accent, goal, goalNote }`)을 붙여넣으면 그 값을 그대로 쓰고, 세 값을 표로 보여 준 뒤 바로 A로 넘어간다. 다시 묻지 않는다.
- 붙여넣은 결과가 없으면 사전 인터뷰 링크를 주고 결과를 붙여넣어 달라고 한다. 사용자가 여기서 답하겠다고 하면 아래 표대로 하나씩 묻는다.
- 키컬러가 표에 없는 색(웹사이트에서 "직접 고른 색")이면 그 hex를 그대로 `accent`에 쓴다. 프로필의 `accent-ink`는 밝은 바탕에서 글자로 읽히도록(대비 4.5:1 이상) 같은 색을 필요한 만큼 어둡게 한 값, `accent-text`는 `accent` 위에서 더 잘 읽히는 쪽(`#121414` 또는 `#FFFFFF`)으로 정한다.
...
**먼저 "기본 정보는 나중에 입력하시겠습니까? (예/아니오)"만 묻는다.**
- **예** → A를 모두 건너뛰고 B로 간다. 프로필 시점은 기본 2029, 나머지 칸은 비워 둔다(자동으로 숨겨진다). 허브 `bio`도 비워 둔다. 나중에 사용자가 "기본 정보 입력할게"라고 하면 A2부터 묻고 `profile.js`·`data.js`를 채워 둘 다 같은 URL로 재게시한다.
- **아니오** → 아래 표대로 A2부터 하나씩 묻는다.
...
게시 전에 `index.html`의 `<title>`을 `meta.siteTitle`과 같게 바꾼다(예: `<title>HS 2029</title>`). 아티팩트 이름이 이 태그로 정해진다.
`index.html`을 아티팩트로 게시하되 `profile.js`(사진이 있으면 사진 파일도)를 **반드시 함께** 올린다.
URL을 받으면 다음 재게시부터는 같은 URL을 유지한다.

## D2. 공유 설정 (허브 게시 뒤)

아티팩트는 처음에 **비공개**로 게시된다. 프로필과 허브를 모두 게시한 뒤, 링크를 단톡방에 올리기 전에 사용자에게 안내한다:
"두 페이지를 각각 열어 오른쪽 위 **Share(공유)** → **링크가 있는 사람**으로 바꿔 주세요. 그래야 단톡방에서 열립니다."
허브 URL을 받은 뒤에는 `profile.js`의 `contact.channels` RUNWAY Hub에 넣고 프로필을 같은 URL로 재게시한다.
